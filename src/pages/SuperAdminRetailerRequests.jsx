...
export default function SuperAdminRetailerRequests() {
  ...
  async function loadRequests() {
    try {
      ...
      const data = await getSuperAdminRetailerRequests({
        search,
        status,
      });
  ...
  async function openRequest(id) {
    try {
      ...
      const data = await getSuperAdminRetailerRequest(id);
  ...
  async function changeStatus(requestId, nextStatus) {
    try {
      ...
      await updateSuperAdminRetailerRequestStatus(
        requestId,
        nextStatus
      );
      ...
      if (selected?.id === requestId) {
        const data = await getSuperAdminRetailerRequest(requestId);
        setSelected(data.request);
      }
    } catch (err) {
      setError(
