...
import {
  getSuperAdminRetailerPayments,
  openSuperAdminPaymentScreenshot,
  downloadSuperAdminPaymentScreenshot,
  updateSuperAdminRetailerPaymentStatus,
} from "../lib/api";

function SuperAdminRetailerPayments() {
  ...
  async function loadPayments(showRefresh = false) {
    try {
      ...
      const response = await getSuperAdminRetailerPayments();
  ...
  async function handleAccept(payment) {
    ...
    try {
      ...
      await updateSuperAdminRetailerPaymentStatus(
        payment.id,
        "paid"
      );
...
export default SuperAdminRetailerPayments;